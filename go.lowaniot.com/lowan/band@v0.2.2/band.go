...
// extra bands.
const (
	AS_923_2    Name = "AS_923_2"
	AS_923_3    Name = "AS_923_3"
	CN_470_S    Name = "CN_470_S"
	CN_470_D20A Name = "CN_470_D20A"
	CN_470_S20B Name = "CN_470_S20B"
	CN_470_D26A Name = "CN_470_D26A"
	CN_470_D26B Name = "CN_470_D26B"
	CN_230_ST   Name = "CN_230_ST"
)
...
// extra bands.
const (
	AS923G2   Name = "AS923-2"
	AS923G3   Name = "AS923-3"
	CN470S    Name = "CN470S"
	CN470D20A Name = "CN470D20A"
	CN470S20B Name = "CN470S20B"
	CN470D26A Name = "CN470D26A"
	CN470D26B Name = "CN470D26B"
	CN230ST   Name = "CN230ST"
)
...
type Band interface {
	...
	AddChannelForUplink(frequency, minDR, maxDR int) error

	AddChannelForDownlink(frequency, minDR, maxDR int) error
	...
	//GetRX1FrequencyForJoinFrequency get the join accept frequency for Rx1
	GetRX1FrequencyForJoinFrequency(joinFrequency int) (int, error)
	...
	GetCFList(protocolVersion string, joinFrequency int) *lorawan.CFList
	...
	// 用于强制修改 RX2Frequency 满足 CN470S 等招抄频段的需求，
	// frequency 是上行频点，用于计算 RX1Frequency，
	// 这个参数不会作用于标准频段
	GetRX2Frequency(frequency int, isABP bool) int
	GetRX2DataRate(uplinkDR int, drOffset int) int

	//增加是否join频点的判断
	CheckJoinFrequency(frequency int) bool

	// CN470 RX2 离散的临时解决方案。
	// 不够优雅，后续需与 GetRX2Frequency() 一并考虑优化方案。
	GetRandomDownLinkFrequency() int
}
...
func (b *band) AddChannelForUplink(frequency, minDR, maxDR int) error {
	if !b.supportsExtraChannels {
		return errors.New("lorawan/band: band does not support extra channels")
	}
...
func (b *band) AddChannelForDownlink(frequency, minDR, maxDR int) error {
	if !b.supportsExtraChannels {
		return errors.New("lorawan/band: band does not support extra channels")
	}
...
func (b *band) GetCFList(protocolVersion string, joinFrequency int) *lorawan.CFList {
...
func (b *band) GetRandomDownLinkFrequency() int {
	return b.downlinkChannels[rand.Intn(len(b.downlinkChannels))].Frequency
}
...
func GetConfig(name Name, repeaterCompatible bool, dt lorawan.DwellTime) (Band, error) {
	...
	case AS_923_2, AS923G2:
		return newAS923Band2(repeaterCompatible, dt)
	case AS_923_3, AS923G3:
		return newAS923Band3(repeaterCompatible, dt)
	...
	case CN_470_S, CN470S:
		return newCN470SBand(repeaterCompatible)
	case CN_470_D20A, CN470D20A:
		return newCN470D20ABand(repeaterCompatible)
	case CN_470_S20B, CN470S20B:
		return newCN470S20BBand(repeaterCompatible)
	case CN_470_D26A, CN470D26A:
		return newCN470D26ABand(repeaterCompatible)
	case CN_470_D26B, CN470D26B:
		return newCN470D26BBand(repeaterCompatible)
	case CN_230_ST, CN230ST:
		return newCN230STBand(repeaterCompatible)
...
// GetCN470Name returns the channel plan of the band for CN470 nodes with rp2
func GetCN470ChannelPlan(joinFrequency int) Name {
	switch joinFrequency {
	case 470900000, 472500000, 474100000, 475700000, 504100000, 505700000, 507300000, 508900000:
		return CN470D20A
	case 479900000, 499900000:
		return CN470S20B
	case 470300000, 472300000, 474300000, 476300000, 478300000:
		return CN470D26A
	case 480300000, 482300000, 484300000, 486300000, 488300000:
		return CN470D26B
	}
	if 470300000 < joinFrequency && joinFrequency < 489300000 {
		return CN470
	} else {
		return ""
	}
