...
type cn470SBand struct {
	band
}

func (b *cn470SBand) GetRX1FrequencyForJoinFrequency(joinFrequency int) (int, error) {
	return b.GetRX1FrequencyForUplinkFrequency(joinFrequency)
}

//TODO
func (b *cn470SBand) CheckJoinFrequency(frequency int) bool {
	return true
}

func (b *cn470SBand) Name() string {
	return "CN470S"
}

func (b *cn470SBand) GetDefaults() Defaults {
	return Defaults{
		RX2Frequency:     505300000, // useless
		RX2DataRate:      0,
		MaxFCntGap:       16384,
...
func (b *cn470SBand) GetDownlinkTXPower(freq int) int {
	return 27
}

func (b *cn470SBand) GetPingSlotFrequency(devAddr lorawan.DevAddr, beaconTime time.Duration) (int, error) {
...
func (b *cn470SBand) GetRX1ChannelIndexForUplinkChannelIndex(uplinkChannel int) (int, error) {
	return uplinkChannel % 48, nil
}

func (b *cn470SBand) GetRX1FrequencyForUplinkFrequency(uplinkFrequency int) (int, error) {
...
func (b *cn470SBand) GetRX2Frequency(upFreq int, isABP bool) int {
	// 使用 RX1Frequency 作为 RX2Frequency
	freq, err := b.GetRX1FrequencyForUplinkFrequency(upFreq)
	if err == nil {
		return freq
	}
...
func (b *cn470SBand) GetRX2DataRate(uplinkDR int, drOffset int) int {
	rx1DR, err := b.GetRX1DataRateIndex(uplinkDR, drOffset)
	if err == nil {
		return rx1DR
	}
...
func (b *cn470SBand) ImplementsTXParamSetup(protocolVersion string) bool {
	return false
}

func (b *cn470SBand) GetDefaultMaxUplinkEIRP() float32 {
	return 19.15
}

func newCN470SBand(repeaterCompatible bool) (Band, error) {
	b := cn470SBand{
		band: band{
			...
			// DownlinkTXPower + txPowerOffsets[idx]
			txPowerOffsets: []int{
				-10, // 17
				-11, // 16
				-13, // 14
