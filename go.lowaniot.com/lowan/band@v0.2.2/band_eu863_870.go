...
func (b *eu863Band) GetRX1FrequencyForJoinFrequency(joinFrequency int) (int, error) {
	return b.GetRX1FrequencyForUplinkFrequency(joinFrequency)
}

//TODO
func (b *eu863Band) CheckJoinFrequency(frequency int) bool {
	return true
}
...
func (b *eu863Band) GetRX2Frequency(frequency int, isABP bool) int {
	return b.GetDefaults().RX2Frequency
}

func (b *eu863Band) GetRX2DataRate(uplinkDR int, drOffset int) int {
	...
	if repeatedCompatible {
		b.band.maxPayloadSizePerDR = map[string]map[string]map[int]MaxPayloadSize{
			latest: map[string]map[int]MaxPayloadSize{
				latest: map[int]MaxPayloadSize{ // LoRaWAN 1.0.0, 1.0.1, 1.0.2B, 1.1.0A, 1.1.0B, 1.0.4
					0: {M: 59, N: 51},
					1: {M: 59, N: 51},
					2: {M: 59, N: 51},
					3: {M: 123, N: 115},
